 ...
 * Read a register value from an i2c device
 */
uint16_t generic_read_i2c_register_multi(uint8_t hwaddr, uint8_t regaddr, uint8_t *val, uint16_t len)
{
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (hwaddr << 1) | I2C_MASTER_WRITE, ACK_CHECK_EN);
    ...
    uint8_t retval;
    for(i = 0; i < len; i++) {
    	i2c_master_read_byte(cmd, &retval, ACK_VAL);
    	val[i] = retval;
    }
 ...
 */
esp_err_t generic_write_i2c_register(uint8_t hwaddr, uint8_t regaddr, uint8_t value)
{
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (hwaddr << 1) | I2C_MASTER_WRITE, ACK_CHECK_EN);
    ...
    i2c_master_stop(cmd);
    esp_err_t ret = i2c_master_cmd_begin(I2C_NUM_0, cmd, BEGIN_TIMEOUT);
    i2c_cmd_link_delete(cmd);
 ...
 */
esp_err_t generic_write_i2c_register_word(uint8_t hwaddr, uint8_t regaddr, uint16_t value)
{
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (hwaddr << 1) | I2C_MASTER_WRITE, ACK_CHECK_EN);
